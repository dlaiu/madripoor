<script lang="ts">
	import { game, confirmGerrymandering, clearGroups } from '$lib/game/gameState.svelte.js';
	import { validateGroups, maxGroupSize } from '$lib/game/groups.js';
	import { TILES } from '$lib/board/hex.js';

	const roundNumber = $derived(game.currentRound.roundNumber);
	const groups = $derived(game.currentRound.groups);
	const maxSize = $derived(maxGroupSize(roundNumber));
	const grouped = $derived(groups.reduce((sum, g) => sum + g.tileIds.length, 0));
	const soloCount = $derived(TILES.length - grouped);
	const validation = $derived(validateGroups(groups, roundNumber));
</script>
